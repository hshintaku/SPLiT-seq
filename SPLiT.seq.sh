...
all_output_folder=/home/samba/storage0/Shiomi/20200814MiSeq011
...
core=16
index=/home/samba/storage0/shintaku/sciRNA-seq/star

#input_dir list_of_files output_dir barcode_pattern num_of_cores i/r(index/read)
python /home/samba/storage0/shintaku/sciRNA-seq/fastq_index_edit.py $fastq_folder fastq_list.txt ./ XXXXCCCCCCCCCCCCCCCCCCXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXCCCCCCCCXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXCCCCCCCC 2 r
gunzip *.fastq.gz
gunzip $fastq_folder/*R2*fastq.gz > ./
...
for sample in $(cat $sample_ID); do umi_tools whitelist --stdin ./$sample*R1*paired*gz --bc-pattern=NNNNNNNNNNCCCCCCCCCCCCCCCCCCCCCCCC --set-cell-number=50 --log2stderr > $all_output_folder/$sample.whitelist.txt; done
