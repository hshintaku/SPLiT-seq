...
# modified by Hirofumi Shintaku Sep 2019
# no duplicated umi removal, write annotation to the sam file.
...
def sciRNAseq_count(sample, input_folder, exons, genes, gene_end, gene_annotat, sample_ID):
    import pysam
    input_sam = input_folder + "/" + sample + ".Aligned.sortedByCoord.out.sam"
    output_sam = input_folder +"/" + sample + "_annotated.sam"
    report = input_folder + "/" + sample + ".report"
    count_output = input_folder + "/" + sample + ".count"
#    f3 = open(output_sam+'.csv', 'w')
    ...
    for alnmt in almnt_file:
        ...
        if len(gene_id_intersect) == 1:
            ...
            if len(gene_id_combine) == 1:
                ...
                if len(gene_id_intersect) == 1:
                    gene_id_1 = list(gene_id_intersect)[0]
                    gene_id=list(gene_id_intersect)[0] + "_intron"
                    counts[gene_id] += 1
                    perfect_inter_gene += 1
                    #pre_pos.append("perfect_inter_gene")

                elif len(gene_id_intersect) > 1:
                    #gene_id = find_nearest_gene(alnmt.iv.end_d, gene_id_intersect, gene_end) + "_intron"
                    gene_id_1= find_nearest_gene(alnmt.iv.end_d, gene_id_intersect, gene_end)
                    gene_id = gene_id_1 + "_intron"
                    counts[gene_id] += 1
                    nearest_inter_gene += 1
                    #pre_pos.append("nearest_inter_gene")

                else:
                    # if there no intersection match, then find the union sets
                    if len(gene_id_combine) == 1:
                        gene_id = list(gene_id_combine)[0] + "_intron"
                        gene_id_1=list(gene_id_combine)[0]
                        counts[gene_id] += 1
                        perfect_combine_gene += 1
                        #pre_pos.append("perfect_combine_gene")

                    elif len(gene_id_combine) > 1:
                        gene_id = find_nearest_gene(alnmt.iv.end_d, gene_id_combine, gene_end) + "_intron"
                        gene_id_1=find_nearest_gene(alnmt.iv.end_d, gene_id_combine, gene_end)
                        counts[gene_id] += 1
                        nearest_combine_gene += 1
                        #pre_pos.append("nearest_combine_gene")

                    else:
                        gene_id="_no_feature"
                        counts["_no_feature"] += 1

        lines=alnmt.get_sam_line()
        if gene_id !="_no_feature":
           f2.write(lines + "\tXS:Z:Assigned\tXN:i:1\tXT:Z:" + gene_id)
           f2.write('\n')
#        f3.write(str(len(barcode)) + ", " + str(len(unique_id)))
#        f3.write('\n')
 
    f2.close()
#    f3.close()                    
    print("File name: ", sam_file)
#    print("   Number of duplicated reads:   ", num_dup)
    ...
    print("Sam file analysis finished~" + input_sam)
...
#    cell_annotat_file = "./cell_annotate.txt"
#    report_annotate_file = "./report_annotate.txt"
    ...
    for feature in gtf_file:
        if feature.type == "exon":
            ...
            if feature.attr["gene_id"] in gene_end.keys():
                ...
                gene_end[ feature.attr["gene_id"] ].add(feature.iv.end_d)
        if feature.source =="ERCC":
            gene_n += 1
            genes[ feature.iv ] +=feature.attr["gene_id"]
            gene_count += 1
            
            # for human and mouse gtf file
            message = (feature.attr["gene_id"] + "," +"ERCC" + "," + feature.type +","
                       + feature.attr["transcript_id"] + "," + str(gene_count) + "\n")
